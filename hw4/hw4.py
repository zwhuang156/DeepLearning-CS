...
def simpleRNN(args):
    ...
    if args.cell == 'GRU':
        ...
        RNN_1 = Bidirectional(LSTM(args.hidden_size, 
...
def main():
    ...
    if args.action == 'train':
        ...
        dm.add_testing_data('test_data', args.test_path)
    else:
        raise Exception ('parameter should be "train","semi" or "test"')
    ...
    # testing
        ...
        print("=======  start predict  ========")
        print(np.array(dm.data['test_data']).shape)
        result = model.predict(dm.data['test_data'])
        result = np.around(result)
        print(result.shape)
        
        output = open(args.result_path, "w+")
        s = csv.writer(output,delimiter=',',lineterminator='\n')
        s.writerow(["id","label"])
        for i in range(len(result)):
            s.writerow([str(i), int(result[i][0])]) 
        output.close()
