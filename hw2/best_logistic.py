...
def load_data(train_data_path, train_label_path, test_data_path):
    ...
    X_train = X_train.drop(X_train.columns[53:59], axis = 1)
    X_train = X_train.drop(X_train.columns[31:38], axis = 1)
    ...
    X_test = X_test.drop(X_test.columns[53:59], axis = 1)
    X_test = X_test.drop(X_test.columns[31:38], axis = 1)
...
def train(X_all, Y_all, save_dir):
    ...
    epoch_num = 10000
    save_param_iter = 1000

    # Start training
    total_loss = 0.0
    ada_w_grad = np.zeros((X_train.shape[1],))
    ada_b_grad = np.zeros((1,))
    for epoch in range(1, epoch_num):
        ...
        for idx in range(step_num):
            ...
            ada_w_grad += w_grad**2
            ada_b_grad += b_grad**2
            ada_w = np.sqrt(ada_w_grad)
            ada_b = np.sqrt(ada_b_grad)
...
def train_with_NN(X_all, Y_all):
    ...
    model.save("NN_model/model.h5")
    
    
def infer_with_NN(X_test, output_dir):
    model = load_model("NN_model/model.h5")
    result = model.predict(X_test)
    y = []
    for i in range(result.shape[0]):
        y.append(np.argmax(result[i]))
...
def main(opts):
    ...
    if opts.train:
        ...
        train_with_NN(X_all, Y_all)
    elif opts.infer:
        """infer(X_test, opts.save_dir, opts.output_dir)"""
        infer_with_NN(X_test, opts.output_dir)
    else:
